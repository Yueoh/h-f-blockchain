...
var caName = "ca.org2.example.com";
...
Fabric_Client.newDefaultKeyValueStore({path: store_path}).then((state_store) => {
    ...
    if (user_from_store && user_from_store.isEnrolled()) {
        ...
        return fabric_ca_client.enroll({
          ...
          console.error('Failed to enroll and persist `user2`. Error: ' + err.stack ? err.stack : err);
          throw new Error('Failed to enroll `user2`');
        });
    }
}).then(() => {
    console.log('Assigned the `user2` user to the fabric client ::' + member_user.toString());
}).catch((err) => {
    console.error('Failed to enroll `user2`: ' + err);
});